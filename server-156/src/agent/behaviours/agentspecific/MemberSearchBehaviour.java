	...
	public void action() {
		...
		if (null != msg) {
			if (getDataFromMessage(msg) && getDataFromBlackboard() && !MyStringChecker.isBlank(m_strQuery)) {
				...
				//for (int i = 0; i < m_lsResult.size(); i++) {
				//	System.out.println(m_lsResult.get(i).getSource());					
				//}
				//for (int i = 0; i < diList.size(); i++) {
				//	System.out.println(diList.get(i));
				//}
				//System.out.println("m_nLastAmout:"+m_nLastAmount);
