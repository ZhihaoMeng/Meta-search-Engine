...
public class PieChartPainter {
	...
	private static DefaultPieDataset GetAcaDataSet(List<AcademicResult> result, LangEnv lang)
	{
		DefaultPieDataset dpd = new DefaultPieDataset();
	    Map<AcaSourceName, Double> data=new HashMap<AcaSourceName, Double>();
	    ResultProportionAnalysis.getAcaResultDistributionRate(data, result);
		Iterator<Entry<AcaSourceName, Double>> it=data.entrySet().iterator();
		while(it.hasNext()){
			Entry<AcaSourceName, Double> entry=it.next();
			double val=entry.getValue();
			if(Math.abs(val)>0.000001) {
				dpd.setValue(entry.getKey(), val);
//				System.out.println("acadata dpd item:"+entry.getKey()+" val:"+val);
			}
		}
		
	    return dpd;
	}
	public static JFreeChart GetAcaPieChart(List<AcademicResult> result, LangEnv lang)
	{
		DefaultPieDataset dataSet = GetAcaDataSet(result, lang);
		JFreeChart chart = ChartFactory.createPieChart("", dataSet, false, false, false);
		...
		for (int i = 0; i < dataSet.getKeys().size(); i++) {
			AcaSourceName key=(AcaSourceName) dataSet.getKey(i);
			 Paint color = EngineFactory.getShowColorOfAcaEngine(key);
			 plot.setSectionPaint(key, color);
		}
		/*for (Iterator<String> it=keys.iterator();it.hasNext();) {
			String key=it.next();
