...
public class CreateWordList {
	 ...
	 * @return wordlist
	 */
	public static WordList abstrTermFrequency(String abstr)
	{
		WordList wl = new WordList();
		if (abstr == null || abstr.length() == 0) {
			return null;
		}
		List<String> abstrList = IK.fenci(abstr);
		
		if (abstrList != null && abstrList.size() > 0) {
			for (int i = 0; i < abstrList.size(); i++) {
				int result = CreateWordList.cmp(wl, abstrList.get(i));
				
				if (result == -1) {
					wl.addWord(new Word(abstrList.get(i), 1, 0));
				}else {
					wl.getWord(result).updateCount();
				}
			}
		}
		
		//for (int i = 0; i < wl.getWordListSize(); i++) {
		//	System.out.println("wl1.getWord(i).getwordandfrq"+ wl.getWord(i).getword() + wl.getWord(i).getCount());
		//}
		return wl;
	}
