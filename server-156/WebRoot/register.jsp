<%@page import="db.dbhelpler.JobHelper"%>
<%@page import="db.dbhelpler.ProvincesHelper"%>
...
<%
	...
	List<String[]> provinces = ProvincesHelper.getProvinces();
	List<String[]> jobs = JobHelper.getAllJobs();
%>
	...
	<div id="main-container">
		...
		<form name="myForm" action="" method="post" focus="login">
			...
			<div class="row" id="home-row" >
				<div class="left-col" name="left-col">
					<%=VisibleConstant.getWebpageContent(ContentNames.home, lang) %>
				</div>
				<div class="right-col">
					<div class="right-input">
						<select id="home-input" maxLength=30 name="usersHome" >
							<option value="0"><%=VisibleConstant.getWebpageContent(ContentNames.home_select_prompt, lang) %></option>
							<% if(provinces != null){
						...
						<span id="spanHome" class="input-promt">&nbsp;</span>
					</div>
					<div class="right-prompt">
						<%=VisibleConstant.getWebpageContent(ContentNames.home_prompt, lang) %>
					</div>
				</div>
			</div>
			<div style="clear:both; visibility:hidden;"></div>  
			<div class="row" id="address-job-company-row" >
				<div class="left-col" name="left-col">
					<%=VisibleConstant.getWebpageContent(ContentNames.address, lang) %>
				</div>
				<div class="right-col">
					<div class="right-input">
						<select id="address-input" maxLength=30 name="usersAddress" onchange="createCities()">
							<option value="0"><%=VisibleConstant.getWebpageContent(ContentNames.address_select_prompt, lang) %></option>
								<% if(provinces != null){
									for(String[] string :provinces){
										%><option value = "<%=string[0]%>"><%=string[1]%></option><%
									}
						...
						<select id="city-input" name="usersAddress" >
							<option value="0"><%=VisibleConstant.getWebpageContent(ContentNames.city_select_prompt, lang) %></option>
							
						</select>
						<select id="job-input" maxLength=30 name="usersJob" onchange="createCompanies()"  >
							<option value="0"><%=VisibleConstant.getWebpageContent(ContentNames.job_select_prompt, lang) %></option>
							<% if(jobs != null){
						...
						<select id="company-input" maxLength=30 name="usersCompany" >
							<option value="0"><%=VisibleConstant.getWebpageContent(ContentNames.company_select_prompt, lang) %></option>
						</select>
						<span id="spanHome" class="input-promt">&nbsp;</span>
						
					</div>
					<div class="right-prompt">
						<%=VisibleConstant.getWebpageContent(ContentNames.address_job_company_prompt, lang) %>
					</div>
				</div>
			</div>
			<div style="clear:both; visibility:hidden;"></div>  
