...
showPages.prototype.createPageHtml = function() {
	var ret="";
	if(!this.isFirstPage){
		ret+="<a class=\"page-jump\" href=\"javascript:void(0)\" onclick=\"acaPageAction(\'"+(this.page-1)+"\')\">"+jsonBundle.lastpage+"</a>";
	}
	for(var i=this.firstShown;i<=this.lastShown;++i){
		if(i==this.page){
			ret+="<span class=\"page-elem\">"+i+"</span>";
		}else{
			ret+="<a class=\"page-elem\" href=\"javascript:void(0)\" onclick=\"acaPageAction(\'"+i+"\')\">"+i+"</a>";
		}
	}
	ret+="<a class=\"page-jump\" href=\"javascript:void(0)\" onclick=\"acaPageAction(\'"+(this.page+1)+"\')\">"+jsonBundle.nextpage+"</a>";
	return ret;
}
...
function setImgLike(n,title, abstr, year, sources, link, like, citecount, authors, score){
	 var element = document.getElementById("likeshow"+n);
	 var reference = document.getElementById("dislikeshow"+n);
	 var esrc = element.src;
	 var index = esrc.lastIndexOf('/');
	 esrc = esrc.substr(index+1);
	 var refsrc = reference.src;
	 var refindex = refsrc.lastIndexOf('/');
	 refsrc = refsrc.substr(refindex+1);
	 if (esrc=="u_zan.png" && refsrc!="a_nozan.png") {
			element.src = "images/a_zan.png";
			userclick(title, abstr, year, sources, link, like, citecount, authors, score);
		}
		else {
			element.src = "images/u_zan.png";
		}
	 
}
function setImgDisLike(n,title, abstr, year, sources, link, like, citecount, authors, score){
	 var element = document.getElementById("dislikeshow"+n);
	 var reference = document.getElementById("likeshow"+n);
	 var esrc = element.src;
	 var index = esrc.lastIndexOf('/');
	 esrc = esrc.substr(index+1);
	 ...
	 var refindex = refsrc.lastIndexOf('/');
	 refsrc = refsrc.substr(refindex+1);
	 if (esrc=="u_nozan.png" && refsrc!="a_zan.png") {
			element.src = "images/a_nozan.png";
			userclick(title, abstr, year, sources, link, like, citecount, authors, score);
		}
		else {
			element.src = "images/u_nozan.png";
		}
}

function changeRank(num){
	var elem = document.getElementById("rankbox"+num);
	var img = elem.style.backgroundImage;
	var index = img.lastIndexOf('/');
	img = img.substr(index+1);
	if(img=="rank_check.png)"){
		elem.style.backgroundImage = "url(images/rank_uncheck.png)";
	}else{
		elem.style.backgroundImage = "url(images/rank_check.png)";
	}	
}

function autoRank(){
	var result="";
	var elem = document.getElementById("rankChose");
	var all = elem.getElementsByTagName("div");
	for(var i=0;i<all.length;i++){
		var img = all[i].style.backgroundImage;
		var index = img.lastIndexOf('/');
		img = img.substr(index+1);
		if(img=="rank_check.png)"){
				
			if(all[i].innerText=="引用次数"){
				result=result+"1 ";
			}else if(all[i].innerText=="影响因子"){
				result=result+"2 ";
			}else if(all[i].innerText=="发表年限"){
				result=result+"3 ";
			}else if(all[i].innerText=="相似性"){
				result=result+"4 ";
			}
		}
	}
...
function autoRankFilt(s){
	...
	document.forms.myForm.action="academic.action";
	var elem= document.getElementById("autoRank");
	if(null!=elem) elem.value = s;//首页提交新请求时需要将page置为1
	document.forms.myForm.submit();
}
